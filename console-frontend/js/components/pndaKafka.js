        ...
        if (metricData.length > 0) {
          ...
          angular.forEach(metricData, function(metric) {
            if (metric.name.endsWith(".health") && !metric.name.includes(".topics.")) {
              ...
              var match;
              var brokerId, broker;
              scope.latestTopicList = $window.localStorage.getItem('currentTopics');

              //compare scope.topics with latestTopicList to check if any topic is deleted
              if(metric.name.endsWith(".available.topics")){
                if(scope.topics && scope.latestTopicList){
                   for(topic in scope.topics){
                     if(scope.latestTopicList.indexOf(topic) === -1){
              ...
              if ((match = metric.name.match(/^kafka\.brokers\.(\d+)\.topics\.(.*)\.((?:BytesInPerSec|BytesOutPerSec|MessagesInPerSec))\.(.*)/i)) !== null) {
                ...
                scope.serverTime = $window.localStorage.getItem('serverTime');
                scope.timeDiff = scope.serverTime - timestamp;
                //Add topic for display purpose into scope.topics only when that topic present in latestTopicLatest
                if(scope.latestTopicList.indexOf(topic) !== -1){
